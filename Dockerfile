...
#
# $ sudo docker build . -t aria2-mingw
#
# After successful build, windows binary is located at
# /aria2-nolimit/src/aria2c.exe.  You can copy the binary using following
# commands:
#
# $ sudo docker run --rm -it -v /path/to/dest:/out aria2-mingw cp /aria2-nolimit/src/aria2c.exe /out
...
ADD https://api.github.com/repos/starreeze/aria2-nolimit/git/$ARIA2_REF version.json
COPY ./ /aria2-nolimit
RUN ls -l /aria2-nolimit
RUN cd /aria2-nolimit && autoreconf -i && ./mingw-config && make -j$(nproc) && \
